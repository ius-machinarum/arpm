...
def test_no_model_loader_in_episode_free_tools():
    files=['tokenizer_snapshot_audit.py','inspect_vector_artifact.py','freeze_vector_artifact.py','search_token_matched_roles.py','audit_prompts_tokenizer_only.py','run_episode_free_audit.py']
    banned={'AutoModel','AutoModelForCausalLM','pipeline','from_pretrained_model'}
    for fn in files:
        txt=(ROOT/fn).read_text()
        for b in banned:
            assert b not in txt, (fn,b)
...
def test_final_tokenizer_audit_consumes_search_selection():
    ...
    assert '--matched-role-clauses' in txt
    assert "selected['selected_clauses']" in txt

def test_tokenizer_snapshot_rejects_weights():
    txt=(ROOT/'tokenizer_snapshot_audit.py').read_text()
    assert '.safetensors' in txt and '.bin' in txt
    assert 'Model-weight-like file found' in txt
...
def test_pins_consistent():
    ...
    for fn in ['search_token_matched_roles.py','audit_prompts_tokenizer_only.py','tokenizer_snapshot_audit.py']:
        txt=(ROOT/fn).read_text()
        assert rev in txt, fn
        assert sha in txt, fn
