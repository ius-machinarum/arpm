"""Deterministic tokenizer-only search for A/C/D1 role clauses.

NO model weights are loaded. This script searches ONLY the precommitted variants in
role_clause_variants.py. It writes a selection only if the same variant triplet makes
all 12 prompt families exactly position-matched at both primary and recovery renders.
"""
...
def suffix_added(base_ids, gen_ids):
    if gen_ids[:len(base_ids)] != base_ids:
        raise AssertionError("generation prompt is not a pure suffix")
    return gen_ids[len(base_ids):]

def user_setup_with(f, condition, clause):
...
def messages_with(f, condition, clause, include_recovery):
    out=[
        {"role":"system","content":pmg.SYSTEM},
        {"role":"user","content":user_setup_with(f,condition,clause)},
...
def audit_combo(tok, idxs):
    clauses={c: ROLE_VARIANTS[c][i] for c,i in zip(("A","C","D1"),idxs)}
    family_reports={}
    for f in pmg.FAMILIES:
        rows={}
        suffixes=[]
        for c in pmg.CONDITIONS:
            primary=messages_with(f,c,clauses[c],False)
            no_gen=render_ids(tok,primary,False)
            with_gen=render_ids(tok,primary,True)
            ps=suffix_added(no_gen,with_gen)
            full=messages_with(f,c,clauses[c],True)
            full_no=render_ids(tok,full,False)
            full_ids=render_ids(tok,full,True)
            if full_ids[:len(with_gen)] != with_gen:
                return None
            ...
            rows[c]={
                ...
                "setup_n":len(tok.encode(user_setup_with(f,c,clauses[c]),add_special_tokens=False)),
            }
            suffixes.append((tuple(ps),tuple(rs)))
        if len({rows[c]["primary_prefix_n"] for c in pmg.CONDITIONS}) != 1: return None
        if len({rows[c]["recovery_full_n"] for c in pmg.CONDITIONS}) != 1: return None
...
def main():
    ...
    args=ap.parse_args()
    if not args.local_tokenizer_dir and args.revision != PINNED_REVISION:
        raise SystemExit("Refusing non-pinned remote revision")
    if args.tokenizer_json:
        d=sha256(args.tokenizer_json)
        if d != EXPECTED_TOKENIZER_SHA256:
            raise AssertionError(f"tokenizer.json SHA mismatch: {d}")
    if args.local_tokenizer_dir and not args.tokenizer_json:
        raise SystemExit('--tokenizer-json is required with a local tokenizer snapshot')
    ...
    if not hits:
        raise SystemExit("NO EXACT MATCH among precommitted semantic variants. Redesign before live inference; do not add padding ad hoc.")
    ...
    payload={
        ...
        "selected_indices":{"A":idxs[0],"C":idxs[1],"D1":idxs[2]},
    ...
    print(json.dumps({k:payload[k] for k in ["status","selected_indices","n_exact_hits"]},indent=2))
