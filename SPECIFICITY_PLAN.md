...
**Status:** prospective / episode-free. No model activations have been observed.
...
Freeze **100** random directions before any activation data.
...
- expected SHA-256 of concatenated raw float32 bytes:
...
## 3. Naive semantic Mold control
Artifact candidate:
`vectors_naive_faithful_pc5000.pt`
from the same external mirror and immutable revision as the trained vector.

The exact byte hash, object key, tensor shape, layer convention and selected naive Mold layer must be frozen from artifact metadata **before live inference**.

Analysis uses the same unit-normalisation and measurement span.
