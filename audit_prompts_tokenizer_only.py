"""Final tokenizer-only structural audit. NO model weights/inference.

This consumes the deterministic role-clause selection produced by
search_token_matched_roles.py, so the final audit checks the SAME clauses that would
be frozen for the experiment rather than silently falling back to draft clauses.
"""
...
def main():
    ...
    if selected.get('status')!='TOKEN_MATCH_PASS': raise AssertionError('role-clause selection is not a passing frozen selection')
    clauses=selected['selected_clauses']
    if set(clauses)!={'A','C','D1'}: raise AssertionError('condition set mismatch')
    tok=load_tokenizer(args.local_tokenizer_dir)
    report={'status':'FINAL_TOKENIZER_AUDIT_PASS','revision':args.revision,'tokenizer_sha256':digest,'selected_indices':selected['selected_indices'],'families':{}}
    for f in pmg.FAMILIES:
        ...
        for c in pmg.CONDITIONS:
            ...
            rows[c]={'primary_prefix_n':len(gen),'primary_no_gen_n':len(no),'recovery_full_n':len(full_gen),'primary_suffix_ids':ps,'recovery_suffix_ids':rs}
            suffixes.append((tuple(ps),tuple(rs)))
        if len({rows[c]['primary_prefix_n'] for c in pmg.CONDITIONS})!=1: raise AssertionError(f'{f.family_id}: primary length mismatch')
        if len({rows[c]['recovery_full_n'] for c in pmg.CONDITIONS})!=1: raise AssertionError(f'{f.family_id}: recovery length mismatch')
    ...
    print(f"PASS: {len(report['families'])} families use the selected clauses with exact position matching")
